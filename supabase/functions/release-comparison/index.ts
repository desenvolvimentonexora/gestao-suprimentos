...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const body = (await req.json()) as Partial<ReleaseBody>
    const { comparisonId, decision, paymentConditionNote, rejectionReason } = body

    if (!comparisonId || (decision !== 'released' && decision !== 'rejected')) {
    ...
    if (!hasApprovePermission) return jsonResponse({ error: 'Sem permissão para liberar.' }, 403)

    const serviceRoleKey = Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')!
    const adminClient = createClient(supabaseUrl, serviceRoleKey)

    const { error: rpcError } = await adminClient.rpc('fn_release_comparison', {
      p_comparison_id: comparisonId,
      p_decision: decision,
      p_decided_by: userId,
      p_payment_condition_note: paymentConditionNote ?? null,
